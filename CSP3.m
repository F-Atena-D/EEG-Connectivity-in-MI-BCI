...
% classifying RH and LH EEG data in Physionet dataset
...
for d = 1:109 % Subjects
    for ii = 1:n
        ...
        for tr = 1:length(tridx)
            cLtr = squeeze([DL(d, tridx(tr), :, :)]);
            cRtr = squeeze([DR(d, tridx(tr), :, :)]);
...
cspFeaturesRtrd(ii, d) = {cspFeaturesRtr};
        ...
        for tr = 1:length(tsidx)
            cLtr = squeeze([DL(d, tsidx(tr), :, :)]);
            cRtr = squeeze([DR(d, tsidx(tr), :, :)]);
...
%%
save('cspLR3.mat', "cspFeaturesRtsd", "cspFeaturesLtsd", "cspFeaturesRtrd", "cspFeaturesLtrd");
